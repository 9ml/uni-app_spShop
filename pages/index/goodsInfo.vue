<template>
	<view>
		...
		<!-- 此商品的拼团信息 -->
		<view class="friends-main" v-if="isPTing">
			<view class="fm-title">
				此商品还没有拼团新消息哦，快去创建一个团吧~
			</view>
		</view>
		<view class="friends-main" v-for="(t,t_idx) in ptingInfo" :key="t_idx">
			<view class="fm">
				<view class="fm-item">
					<view class="fi-left">
						<view class="fl-header">
							<image :src="t.userImg" mode=""></image>
						</view>
						<view class="fl-name">
							{{t.userName}}
						</view>
					...
					<view class="fi-middle">
						...
						<view class="fm-bom">
							剩余{{t.dayTimes}}天{{t.hoursTimes}}时{{t.minutesTimes}}分{{t.secondsTimes}}秒后结束
						</view>
					</view>
					<view class="fi-right">
						<view class="fr-btn" @tap="goPTing(t.id,t.ptGoodsTableId)">
							去拼单
						</view>
					</view>
				</view>
			...
			<view class="type-main">
				...
				<view class="tm-color">
					...
					<view class="tc-main" v-if="isHaveSize">
						<view class="tm-item" :class="current_s === size_idx ? 'ac' : ''" v-for="(size_item,size_idx) in sizeList" @tap="changeSize(size_idx,size_item.goodsSize)"
						 :key="size_idx">
							{{size_item.goodsSize}}
						</view>
					</view>
		...
		<!-- 选择拼团类型 -->
		<uni-popup ref="ptType" type="bottom" :custom="true">
			<view class="pt-main">
				<view class="pm-title">
					选择拼团类型
				</view>
				<view class="ptMs" v-for="(p,p_idx) in ptArr" :key="p_idx" @tap="chosePType(p_idx)">
					<view class="pt-item">
						<view class="pi-left">
							{{p.ptPersonNum}}人拼团
						</view>
						<view class="pi-icon" :class="[puCurrent === p_idx ? 'pi-ac' : '']"></view>
					</view>
				</view>
				<view class="pt-btn" @tap="suerPt()">
					确定
				</view>
			</view>
		</uni-popup>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			let getArr = options.arr.split(",");
			let getNum = options.num.split(",");
			getArr.forEach((val, i) => {
				this.ptArr.push({
					ptId: getArr[i],
					ptPersonNum: getNum[i]
				})
			})
		...
		methods: {
			...
			// 此商品正拼团信息
			getPtingInfo() {
				this._Api.showPtingGoods({
					ptGoodsTableId: this.ptArr[0].ptId
				}, res => {
					console.log(res);
					if (res.meta.code === 200) {
						if (res.data.length === 0) {
							...
							res.data.forEach((i) => {
								if (i.userImg.indexOf("http") != -1) {} else {
									i.userImg = this.imgUrl + i.userImg
								}
								this.myTools.timesChange(i.sustainTime - timestamp)
								i.dayTimes = this.myTools.timesChange(i.sustainTime - timestamp)[0];
								i.hoursTimes = this.myTools.timesChange(i.sustainTime - timestamp)[1];
								i.minutesTimes = this.myTools.timesChange(i.sustainTime - timestamp)[2];
								i.secondsTimes = this.myTools.timesChange(i.sustainTime - timestamp)[3];
								i.timer = setInterval(() => {
									i.secondsTimes--
									if (i.secondsTimes <= 0) {
										i.minutesTimes--
				...
				this._Api.evaluate({
					...
					if (res.meta.code === 200) {
						...
						if (res.data.length !== 0) {
							...
							console.log(this.evaluateFirst)
						} else {
			...
			goBuy() {
				if (!this.userId) {
					...
					} else if (this.buyType == "去拼单") {
						uni.setStorageSync("goodsId", this.goodsId);
						uni.setStorageSync("goodsTypeId", this.goodsTypeId);
						uni.setStorageSync("goodsNums", this.buyNum);
						uni.setStorageSync("buyType", this.buyType);
						uni.setStorageSync("ctTypeId", this.ctTypeId);
						uni.setStorageSync("ptType", this.ptType);
						// 拼团购买
						uni.navigateTo({
							url: "./submitOrderCT"
						})
			...
			suerPt() {
				...
				uni.setStorageSync("goodsNums", this.buyNum);
				uni.setStorageSync("buyType", this.buyType);
				uni.setStorageSync("ptType", this.ptArr[this.puCurrent].ptId);
				// 拼团购买
				uni.navigateTo({
					url: "./submitOrderPT"
				})
