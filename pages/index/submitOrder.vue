...
<script>
	...
	export default {
		...
		methods: {
			...
			// 商品信息
			getGoodsInfos(){
				this._Api.getConfirmGoodsInfo({
					id : this.goodsTypeId
				},res=>{
					console.log(res);
					if(res.meta.code === 200){
						res.data.goodsMainImg = this.imgUrl + res.data.goodsMainImg
						res.data.lastPrice = res.data.goodsPrice - res.data.couponPrice
						this.userChoseGoodsInfo = res.data;
						this.totalPrice = (res.data.goodsPrice - res.data.couponPrice) * this.goodsNums
					}else{
						uni.showToast({
							title:res.meta.msg,
							icon:'none',
			...
			getPrice(){
				this.totalPrice = (this.userChoseGoodsInfo.goodsPrice - this.userChoseGoodsInfo.couponPrice) * this.goodsNums
			},
			// 选择配送方式
			ChosePsStyle(){
			...
			submitOrder(){
				if(this.isSetAdress){
					...
					var allPrice = this.userChoseGoodsInfo.goodsPrice * this.goodsNums
					this._Api.confirmOrder({
						userId : this.userId,
						consigneeId : this.consigneeId,
						propertyId : this.goodsTypeId,
						goodsAmount : this.goodsNums,
						orderPrice : allPrice,
						couponPrice : this.userChoseGoodsInfo.couponPrice,
						orderAmount : this.totalPrice,
						promType : this.buyType,
			...
			choseMyAdress(){
				uni.setStorageSync("choseAdressType",0);
				uni.navigateTo({
					url:"../mine/myAdress"
				})
			}
